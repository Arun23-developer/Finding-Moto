...
const getBuyerId = (req: AuthRequest): mongoose.Types.ObjectId => req.user!._id as mongoose.Types.ObjectId;

const serializeCartItem = (item: any) => {
...
const refreshCartSnapshot = async (item: any): Promise<void> => {
  const productDoc = item.product && typeof item.product === 'object' ? item.product : null;
...
const getValidatedProduct = async (productId: string) => {
...
const hasValidationError = (
  result: Awaited<ReturnType<typeof getValidatedProduct>>
): result is { error: { status: number; message: string } } => {
  return 'error' in result;
};

export const getCartItems = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const cartItems = await Cart.find({ buyer: getBuyerId(req) })
...
export const addCartItem = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    let cartItem = await Cart.findOne({ buyer: buyerId, product: product._id });
...
export const updateCartItemQuantity = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const cartItem = await Cart.findOne({ _id: id, buyer: buyerId });
    if (!cartItem) {
...
export const removeCartItem = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const buyerId = getBuyerId(req);
    const cartItem = await Cart.findOneAndDelete({ _id: req.params.id, buyer: buyerId });
