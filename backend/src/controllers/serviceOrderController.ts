...
const isMechanicRole = (role: string): boolean => role === 'mechanic';
const isBuyerRole = (role: string): boolean => role === 'buyer';

const getOrderAudience = (role: string) => {
  if (isBuyerRole(role)) return 'buyer';
  return 'seller';
};

const getMechanicDisplayName = (mechanic: any) => {
  if (!mechanic) return 'Mechanic';
  return mechanic.workshopName || `${mechanic.firstName || ''} ${mechanic.lastName || ''}`.trim() || 'Mechanic';
};

export const createServiceOrder = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const service = await Service.findOne({ _id: serviceId, active: true, productStatus: 'ENABLED' });
    if (!service) {
...
export const getBuyerServiceOrders = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const buyerId = req.user!._id;
    const orders = await ServiceOrder.find({ buyer: buyerId })
      .sort({ createdAt: -1 })
...
export const getMechanicServiceOrders = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const mechanicId = req.user!._id;
    const orders = await ServiceOrder.find({ mechanic: mechanicId })
      .sort({ createdAt: -1 })
...
const ACTIONS: Record<string, { nextStatus: ServiceOrderStatus; roles: Array<'buyer' | 'mechanic'>; allowedCurrent: string[]; buyerMessage: string; mechanicMessage: string }> = {
  accept: {
...
export const updateServiceOrderStatus = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const user = req.user!;
    const { id } = req.params;
    ...
    if (!actionConfig.roles.includes(user.role as 'buyer' | 'mechanic')) {
    ...
    emitOrderWorkflowEvent({
      ...
      actorRole: user.role as 'buyer' | 'mechanic' | 'delivery_agent' | 'system',
    });
...
function getOrderStatusTitle(status: string) {
  return SERVICE_ORDER_STATUS_LABELS[normalizeServiceOrderStatus(status)] || status;
}
