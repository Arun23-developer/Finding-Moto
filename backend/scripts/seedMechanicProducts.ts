...
async function ensureMechanicAccount() {
  let mechanic = await User.findOne({ email: MECHANIC_EMAIL, role: 'mechanic' }).select('+password');
...
async function seedMechanicProducts(): Promise<void> {
  try {
    ...
    for (const product of SAMPLE_PRODUCTS) {
      const existing = await Product.findOne({ seller: mechanic._id, sku: product.sku });
