...
type DeliveryTimelineStatus = 'ASSIGNED' | 'PICKED_UP' | 'IN_TRANSIT' | 'DELIVERED' | 'FAILED';
...
const ORDER_PATTERNS: TimelineStatus[][] = [
...
const createStatusHistory = (statuses: TimelineStatus[], startAt: Date, stepHours: number): StatusStep[] => {
...
const getDeliveryTimeline = (orderStatus: TimelineStatus): DeliveryTimelineStatus[] => {
  switch (orderStatus) {
    case 'completed':
    case 'delivered':
...
const createDeliveryHistory = (statuses: DeliveryTimelineStatus[], startAt: Date): { status: DeliveryTimelineStatus; changedAt: Date }[] => {
  return statuses.map((status, index) => ({
    status,
...
const finalOrderStatusForIndex = (orderIndex: number): TimelineStatus => {
...
const randomPastDate = (minDaysAgo: number, maxDaysAgo: number): Date => {
...
const seedHistoricalMarketplace = async (): Promise<void> => {
  try {
    await mongoose.connect(config.mongoURI);
    console.log('✓ MongoDB Connected\n');

    const [buyersRaw, sellersRaw, agentsRaw, productsRaw] = await Promise.all([
      User.find({ role: 'buyer', isActive: true, isEmailVerified: true }).select('_id email address').lean(),
      User.find({ role: 'seller', approvalStatus: 'approved', isActive: true }).select('_id email shopName').lean(),
      User.find({
        role: 'delivery_agent',
    ...
    const buyers = buyersRaw as SeedBuyer[];
    const sellers = sellersRaw as SeedSeller[];
    const agents = agentsRaw as SeedAgent[];
    const products = productsRaw as SeedProduct[];
    ...
    const previousOrders = await Order.find({ notes: { $regex: `^${escapeRegex(HISTORY_MARKER)}` } }).select('_id').lean();
    const previousOrderIds = previousOrders.map((order) => order._id);

    if (previousOrderIds.length > 0) {
      await Delivery.deleteMany({ orderId: { $in: previousOrderIds } });
      await Review.deleteMany({ comment: { $regex: `^${escapeRegex(HISTORY_MARKER)}` } });
    ...
    const deliveryDocuments: Array<Record<string, unknown>> = [];
    const reviewDocuments: Array<Record<string, unknown>> = [];
    ...
    const insertedOrders = await Order.collection.insertMany(orderDocuments);
    const insertedOrderIds = Object.values(insertedOrders.insertedIds).map((value) => value as mongoose.Types.ObjectId);

    for (let i = 0; i < deliveryDocuments.length; i++) {
      deliveryDocuments[i].orderId = insertedOrderIds[i];
    }

    await Delivery.collection.insertMany(deliveryDocuments);
    await Review.collection.insertMany(reviewDocuments);
